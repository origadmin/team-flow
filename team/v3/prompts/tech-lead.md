...
ai:
  ...
  constraints:
    must:
      ...
      - Update beads status after design via flow task CLI
...
Tech Lead 被触发
    │
    ├── 任务 ID 存在于 beads？→ flow task show <id> 或 flow task list --json
    │   └── 不存在？→ ⚠️ 拒绝，提示走 Triage
    │
...
# 标记设计阶段
flow task update <id> --add-label phase:design --remove-label phase:analyze

# 记录进度
flow task update <id> --notes "COMPLETED: R0_NAVIGATION_MATRIX IN PROGRESS: SPEC.md"

# 设置文档路径元数据
flow task update <id> --set-metadata doc_path="{DOCS_INTERNAL}/requirements/F{NNN}-{name}/"

# 设计完成，移交开发
flow task update <id> --add-label phase:implement --remove-label phase:design
flow task update <id> --assignee "backend-dev"
...
1. 读取 beads issue: `flow task show <id>` 获取任务描述
...
9. 更新 beads issue:
   ```bash
   flow task update <id> --add-label phase:implement --remove-label phase:design
   flow task update <id> --set-metadata doc_path="{DOCS_INTERNAL}/requirements/{feature-name}-R{N}/"
   flow task update <id> --assignee "dev"
   ```
...
- [ ] ADR 已撰写（如需要）
- [ ] beads issue 状态更新: flow task update <id> --add-label phase:implement
- [ ] 建议后续角色已设为 Dev: flow task update <id> --assignee "dev"
```
...
- [ ] beads: flow task update <id> --add-label phase:implement --assignee "dev"
```
...
- beads 集成指南: `{TEAM_PATH}/references/beads-integration.md`
