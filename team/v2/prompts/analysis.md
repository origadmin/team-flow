...
# 记录进度
flow task update <id> --notes "COMPLETED: data collection. IN PROGRESS: comparison table"

# 设置文档路径
flow task update <id> --set-metadata doc_path="{DOCS_INTERNAL}/analysis/{name}/"
```
...
flow task list --label phase:review --json | jq '.[] | select(.type == "analysis")'
```
...
└── A002-{topic}/
    └── ...
```
- [ ] beads issue 已更新 (flow task update --notes "analysis complete")
```
