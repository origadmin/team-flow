...
| 职责 | 说明 |
|------|------|
| 验证 | 执行测试，确Bug 修复 / Feature 实现 |
| Error Report | Bug 修复后撰写错误报告（含根经验|
| LESSON-NEEDED | 发现值得记录标记 |
...
> **Task 已创* Entry Flow 中已执行 `flow task create`。现在分析用户输入，确定类型/优先分发角色
...
分发Agent 时，Triage 使用以下模板构prompt
...
3. **Sub-agent MUST read Layer 2 files before starting work** 启动后第一件事是加Layer 2
