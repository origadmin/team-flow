...
ai:
  ...
  constraints:
    must:
      ...
      - Update beads status after verification via bd CLI
...
- [ ] Test report output
- [ ] beads status updated (bd update <id> --add-label phase:review)
```
...
6. Bug submission -> bd create "Bug: ..." -t bug -p 0 --external-ref "B{NNN}" --json
7. Dev fixes bug
...
|-------|--------|----------|
| beads issue | `bd show <id> --json` | Yes |
...
| Bug issues | `bd create "Bug: ..." -t bug --json` | beads issue |
