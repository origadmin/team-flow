...
- [ ] [v2] 数据流追踪: RCA.md 包含"数据流追踪"章节（涉及API/权限/状态/交互的Bug必须）
- [ ] [v2] 数据流追踪: 断点已定位到具体环节（不是"可能是xxx"）
- [ ] [v2] 真实场景验证: TEST_CASE.md 包含真实场景验证（非仅mock测试）
- [ ] [v2] 真实场景验证: 覆盖默认值/零值/边界条件
- [ ] [v2] 真实场景验证: 验证结果为通过（不是"已编写"）
- [ ] [v2] 运行时验证: 后端Bug需HTTP请求验证通过（非仅UseCase单元测试）
- [ ] [v2] 运行时验证: 前端Bug需页面可正常渲染+核心交互可工作（非仅组件mock测试）
- [ ] [v2] 运行时验证: 修复后的完整链路已验证（非仅断点环节）
- [ ] task-pool.md 状态已更新
