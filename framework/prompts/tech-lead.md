...
ai:
  ...
  constraints:
    must:
      ...
      - Update Task Pool: After design, set status to Doing, 建议后续角色 to Dev
...
| 任务 | task-pool ID | 资产目录格式 | 产出物 |
|------|-------------|-------------|--------|
| Feature | F{NNN} | {feature-name}-R{N}/ | SPEC.md, AC.md, R1-R4 |
| Change | C{NNN} | C{NNN}-R{N}/ | CHANGE_EVAL.md |
| Analysis | A{NNN} | {name}/ | INDEX.md, COMPARISON.md |
...
9. 更新 task-pool.md：状态 → Doing，建议后续角色 → Dev
...
- [ ] Task Pool: .team/task-pool.md | 状态: Doing | 后续: Dev
```
